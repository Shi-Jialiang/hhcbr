...
from utility import Datasets, NutritionData, NutritionMetric
from models.HHCBR import HHCBR
...
def get_cmd():
    ...
    parser.add_argument("-m", "--model", default="HHCBR", type=str, help="which model to use, options: HHCBR")
...
def main():
    ...
    assert paras["model"] in ["HHCBR"], "Pls select models from: HHCBR"
    ...
    conf["num_ingredients"] = dataset.num_ingredients
    ...
    for lr, l2_reg, item_level_ratio, bundle_level_ratio, bundle_agg_ratio, embedding_size, num_layers, c_lambda, c_temp in \
        ...
        # model
        if conf['model'] == 'HHCBR':
            model = HHCBR(conf, dataset.graphs).to(device)
        else:
            raise ValueError("Unimplemented model %s" %(conf["model"]))

        optimizer = optim.Adam(model.parameters(), lr=lr, weight_decay=conf["l2_reg"])
        scheduler = optim.lr_scheduler.StepLR(optimizer, step_size=50, gamma=0.5)  # 每50个epoch学习率减半
        ...
        for epoch in range(conf['epochs']):
            ...
            for batch_i, batch in pbar:
                ...
                if (batch_anchor+1) % test_interval_bs == 0:  #  and epoch>100 in mealrec+H
                    metrics = {}
                    metrics["val"] = test(model, dataset.val_loader, conf, test=False)
...
def health_rerank(pred_b, topk, fsa_scores):
    """
    pred_b: [bs, num_bundles]
    返回 reranked_pred_b: [bs, num_bundles] （只修改前 topk 的排序）
    """
    # 1. 取每个用户pred_b最高的topk index (模型原始推荐)
    _, topk_idx = torch.topk(pred_b, topk, dim=1)   # [bs, topk]

    # 2. 取这些bundle的健康分数
    # fsa_scores: [num_bundles]
    fsa_for_topk = fsa_scores[topk_idx]   # [bs, topk]

    # 3. 按健康排序（FSA越低越健康）
    rerank_order = torch.argsort(fsa_for_topk, dim=1, descending=False)  # 升序

    # 4. 根据健康排序重排 topk 索引
    reranked_topk_idx = topk_idx.gather(1, rerank_order)   # [bs, topk]

    return reranked_topk_idx


def test(model, dataloader, conf, test=True):
    device = conf["device"]
    if conf["dataset"]=="MealRec+H":
        # 加在 test 函数内 model.eval() 后
        ...
        fsa_scores = torch.tensor(fsa_list, device=device)
        
        # radish
        path = '/home/kazu/bundle/CrossCBR-master/datasets/MealRec+H/healthiness'
        nutritionMetrics = [
            NutritionMetric(path, topK=20),
            NutritionMetric(path, topK=40),
            NutritionMetric(path, topK=80),
        ]
    elif conf["dataset"]=="MealRec+L":
        # 加在 test 函数内 model.eval() 后
        fsa_path = "/home/kazu/bundle/CrossCBR-master/datasets/MealRec+L/healthiness/bundle_fsa.txt"
        with open(fsa_path, "r") as f:
            fsa_list = [float(line.strip()) for line in f.readlines()]
        fsa_scores = torch.tensor(fsa_list, device=device)
        
        # radish
        path = '/home/kazu/bundle/CrossCBR-master/datasets/MealRec+L/healthiness'
        nutritionMetrics = [
            NutritionMetric(path, topK=20),
            NutritionMetric(path, topK=40),
            NutritionMetric(path, topK=80),
    ...
    for users, ground_truth_u_b, train_mask_u_b in dataloader:
        ...
        if test == True:
            for metric in nutritionMetrics:
                metric(pred_b, users)  # 在每个batch累积计算
    
    if test == True:
        for metric in nutritionMetrics:
            metric.print_metric(100000)
...
def get_metrics(metrics, grd, pred, topks):
    ...
    for topk in topks:
        ...
        is_hit = grd[row_indice.view(-1).to(pred.device), col_indice.view(-1).to(pred.device)].view(-1, topk)
...
def get_ndcg(pred, grd, is_hit, topk):
    def DCG(hit, topk, device):
        hit = hit.to(device)/torch.log2(torch.arange(2, topk+2, device=device, dtype=torch.float))
        return hit.sum(-1)
